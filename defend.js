            ...
            if(room.storage){
                enemy = room.storage.pos.findClosestByRange(FIND_HOSTILE_CREEPS,{
                    filter: function(object) {
                        return (object.owner.username != 'Invader' && object.ticksToLive < 1400 && object.body.length >= 40)
                        //return true;
                    }
                })
                ...
                if(enemy){
                    ...
                    if(!Memory.cache.rooms[room_name].atkd_wall){
                        ...
                        // "Memory.cache.rooms[rom].atkd_wall" cache wall under attack
                        console.log('No Wall under attack！')
...
function runAttack(room,attackerName){
    ...
    if(!attacker){
        ...
        if(spawn){
            //console.log(body({ATTACK:4,MOVE:1}))
            console.log(spawn.spawnCreep(body({attack:40,move:10}),attackerName))
        }
        return;
    }
...
function roomRun(room){
    ...
    if(!raAttacker){
        ...
        if(spawn){
            //console.log(body({ATTACK:4,MOVE:1}))
            spawn.spawnCreep(body({ranged_attack:40,move:10}),raAttackerName)
        }
        return;
    }
