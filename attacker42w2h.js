...
module.exports = {
    run: function(flagName, flagStandbyName, flagAttackName, taskName, roomName, status) {
        ...
        const wbody = parse('12t10m28w');
        const hbody = parse('12t10m28h');
        ...
        if(creepCommander && creepPilot && creepSpecialist && creepEngineer){
            ...
            if(!creepCommander){
                autoSpawnCreep(creepCommanderName, roomName, wbody);
            }
            if(!creepPilot){
                autoSpawnCreep(creepPilotName, roomName, wbody);
            }
            if(!creepSpecialist){
                autoSpawnCreep(creepSpecialistName, roomName, hbody);
            }
            if(!creepEngineer){
                autoSpawnCreep(creepEngineerName, roomName, hbody);
            }
        }
    }
}
...
function creepAttack(creepCommander, creepPilot, creepSpecialist, creepEngineer, flagName, flagAttackName){
    if(!creepCommander.pos.isNearBorder && creepCommander.pos.inRangeTo(Game.flags[flagAttackName], 2) == false && !checkPos(creepCommander, creepPilot, creepSpecialist, creepEngineer)){
    ...
    if(creepCommander.pos.inRangeTo(attackFlag, 2)){
        let targets = creepCommander.room.lookForAt(LOOK_STRUCTURES, attackFlag);
        if(targets.length > 0){
            let goals = [{pos:attackFlag.pos, range:1}];
            let retCommander = PathFinder.search(creepCommander.pos, goals,
                {
                    roomCallback: function (roomName) {
                        ...
                        room.find(FIND_CREEPS).forEach(function (creep) {
                            ...
                            ) // 小队成员以外的所有creep设为可行走，更换朝向
                                costs.set(creep.pos.x, creep.pos.y, 0);
                        });
                        return costs;
                    },
                });
                if(creepCommander.moveTo(retCommander.path[0]) == OK) return;
                let retPilot = PathFinder.search(creepPilot.pos, goals,
                    {
                        roomCallback: function (roomName) {
                            let costs = new PathFinder.CostMatrix;
                            let room = Game.rooms[roomName];
                    ...
                    if(creepPilot.moveTo(retPilot.path[0]) == OK) return;
            creepCommander.dismantle(targets[0]);
            creepPilot.dismantle(targets[0]);
            return;
        }
    }
...
Object.defineProperty(RoomPosition.prototype, 'isNearBorder', {
    get: function(){ return this.x<=1 || this.x>=48 || this.y<=1 || this.y>=48; }
})